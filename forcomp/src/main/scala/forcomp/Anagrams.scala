...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences :: (for(o <- occurrences) yield combinations(unOccur(occurrences, o._1))).flatten.distinct
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y match {
      case List() => x
      case _ => subtract(unOccur(x, y.head._1), unOccur(y, y.head._1))
    }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def occAnagrams(occurrences: Occurrences): List[Sentence] = {
      val res = (for( occ <- combinations(occurrences) if occ.nonEmpty ) yield {
        (for( w <- occWords(occ) ) yield {
          if( subtract(occurrences, occ).isEmpty ) List(List(w))
          else {
